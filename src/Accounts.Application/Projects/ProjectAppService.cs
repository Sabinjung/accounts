    ...
    public class ProjectAppService : AsyncCrudAppService<Project, ProjectDto>, IProjectAppService
    {
       ...
       : base(repository)
        ...
        public async Task<Page<ProjectListItemDto>> Search(ProjectQueryParameters queryParameter)
        {
            ...
            query.WhereIf(p => !p.Keyword.IsNullOrWhiteSpace(), p => x => x.Company.DisplayName.Contains(p.Keyword) || x.Consultant.FirstName.ToUpper().Contains(p.Keyword.ToUpper()) || x.Consultant.LastName.ToUpper().Contains(p.Keyword.ToUpper()));
