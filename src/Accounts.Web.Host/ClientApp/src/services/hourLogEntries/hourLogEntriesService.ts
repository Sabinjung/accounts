...
class HourLogEntryService {
  public async getProjectHourLogs(input: GetProjectHourLogsInput): Promise<Array<ProjectHourLogEntries>> {
    ...
    return ([result.data && result.data.result && result.data.result.results && result.data.result.results.map((x: any) => new ProjectHourLogEntries(x)), result.data && result.data.result && result.data.result.recordCount]) || [];
  }
